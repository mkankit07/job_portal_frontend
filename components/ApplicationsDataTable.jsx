...
export default function ApplicationsDataTable({ application }) {
    ...
    const handleDownloadCV = async (name) => {
        const fileUrl = `${process.env.NEXT_PUBLIC_API_BASE_URL}/assets/uploads/${name}`;
    ...
    const columns = [
        ...
        {
            name: 'CV',
            selector: row => <button onClick={() => handleDownloadCV(row?.media_id?.filename)} className=' w-20 py-2 text-xs text-indigo-600 hover:text-white my-2 hover:bg-indigo-600 border border-indigo-600 rounded transition-all duration-700'>Download CV</button>
        },
        {
            name: 'Status',
            selector: row => <p className={`uppercase font-semibold ${row?.status === "approved" ? "text-green-500" : ""}  ${row?.status === "rejected" ? "text-red-600" : ""}`}>{row?.status}</p>,
